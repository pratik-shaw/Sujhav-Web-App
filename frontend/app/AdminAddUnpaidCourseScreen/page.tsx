...
// ---------------------------------------------------------------------------
// Web (Tailwind) rewrite of AdminUnpaidCourseScreen.tsx (Expo). Same SUJHAV
// web design language as AdminPaidCourseScreen / AdminDashboardScreen:
// bg #0a120f, cards #101d17, hairline white/[0.08] borders, emerald-400
// accent, font-serif headings, font-mono for numerals. Uses the same
// /unpaidCourses REST routes as the mobile app — only the transport
// (fetch + FormData) is unchanged, the layout is a proper responsive web
// grid instead of a stacked mobile list.
// ---------------------------------------------------------------------------
...
interface CourseFormState extends Course {
...
export default function AdminUnpaidCourseScreen() {
  ...
  const loadCourses = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE}/unpaidCourses`);
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
      const data = await response.json();
      if (data.success) {
        setCourses(data.data || []);
      } else {
        notify('error', data.message || 'Failed to load courses');
      }
    } catch (error) {
      console.error('Error loading courses:', error);
      notify('error', 'Failed to load courses. Please check your connection.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveCourse = async () => {
    ...
    try {
      ...
      const url = editingCourse
        ? `${API_BASE}/unpaidCourses/${editingCourse._id}`
        : `${API_BASE}/unpaidCourses`;
      ...
      if (data.success) {
        ...
        notify('error', data.message || 'Failed to save course');
      }
    } catch (error) {
      console.error('Error saving course:', error);
      notify('error', 'Failed to save course. Please check your connection.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const deleteCourse = (courseId?: string) => {
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        try {
          setLoading(true);
          const response = await fetch(`${API_BASE}/unpaidCourses/${courseId}`, { method: 'DELETE' });
          if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
          const data = await response.json();
          if (data.success) {
            ...
            notify('error', data.message || 'Failed to delete course');
          }
        } catch (error) {
          console.error('Error deleting course:', error);
          notify('error', 'Failed to delete course. Please check your connection.');
        } finally {
          setLoading(false);
  ...
  const addVideoToCourse = async () => {
    ...
    try {
      setVideoSaving(true);
      const response = await fetch(`${API_BASE}/unpaidCourses/${selectedCourse._id}/videos`, {
        method: 'POST',
  ...
  const updateVideoInCourse = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/unpaidCourses/${selectedCourse._id}/videos/${editingVideo._id}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(videoForm),
  ...
  const deleteVideoFromCourse = (videoId?: string) => {
    ...
    setConfirmState({
      open: true,
      title: 'Delete video',
      message: 'Are you sure you want to delete this video from the course?',
      confirmLabel: 'Delete',
      onConfirm: async () => {
        try {
          setVideoSaving(true);
          const response = await fetch(
            `${API_BASE}/unpaidCourses/${selectedCourse._id}/videos/${videoId}`,
            { method: 'DELETE' }
          );
          if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
          const data = await response.json();
  ...
  const validateCourse = () => {
    ...
    if (courseForm.price < 0) return notify('error', 'Price cannot be negative'), false;
        ...
        >
          {loading && courses.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-white/[0.08] bg-[#101d17] py-24">
              <Spinner />
              <p className="mt-4 text-[13.5px] text-[#8ea79c]">Loading courses…</p>
            </div>
          ) : courses.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-white/[0.08] bg-[#101d17] py-24 text-center">
              <p className="text-[15px] font-semibold text-[#eef4f1]">No free courses yet</p>
              <p className="mt-1.5 max-w-xs text-[13px] text-[#8ea79c]">
                Create your first free course to get it in front of students.
              </p>
              <button
                onClick={() => setShowAddModal(true)}
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Price (₹)">
                <TextInput
                  ...
                  onChange={(v) =>
                    setCourseForm({ ...courseForm, price: Math.max(parseFloat(v) || 0, 0) })
                  }
                  min="0"
                />
              </Field>
            </div>
            <p className="-mt-3 text-[12px] italic text-[#8ea79c]">🎓 Leave price at ₹0 to keep this course free</p>
            ...
            <Field label="Course thumbnail" required>
              <input
                ...
                className="hidden"
                id="unpaid-thumbnail-input"
              />
              <label
                htmlFor="unpaid-thumbnail-input"
...
const CourseCard: React.FC<{
  ...
  <div className="group flex flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-[#101d17] transition-colors hover:border-white/[0.16]">
    <div className="relative h-40 w-full overflow-hidden bg-white/[0.03]">
      ...
      <span
        className={`absolute right-3 top-3 rounded-full border px-3 py-1 text-[12px] font-bold backdrop-blur-sm ${
          !course.price
            ? 'border-emerald-400/40 bg-[#0a120f]/85 text-emerald-300'
            : 'border-amber-300/40 bg-[#0a120f]/85 text-amber-300'
        }`}
      >
